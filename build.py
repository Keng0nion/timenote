...
if __name__ == "__main__":
    ...
    for folder in [BUILD, BUILD / "module-cache", BUILD / "tmp"]:
        folder.mkdir(parents=True, exist_ok=True)
    ENV = dict(os.environ, TMPDIR=str(BUILD / "tmp"), CLANG_MODULE_CACHE_PATH=str(BUILD / "module-cache"))
    ...
    COMMON = [compiler, "-sdk", sdk, "-swift-version", "6", "-target", "arm64-apple-macosx14.0",
              "-file-prefix-map", str(ROOT) + "=/timenote",
              "-debug-prefix-map", str(ROOT) + "=/timenote",
