...
> **当前版本：0.1.2 公开预览版。** 它已经可以用于本地安排、记录和回顾，但不是所有功能都完成的稳定版。**严格提醒尚未启用，睡眠或主程序完全退出后的准时持续响铃尚未实现、尚未验证，请不要用它代替重要闹钟。**
>
> 项目通过数据校验、保存前确认、数据库事务和分层测试提高可靠性；这些措施不等于“完美运行”或“永不丢失数据”。已验证范围、已知警告和未完成事项都在下文说明。

[下载 v0.1.2](https://github.com/Keng0nion/timenote/releases/tag/v0.1.2) · [版本说明](RELEASE_NOTES.md) · [问题反馈](https://github.com/Keng0nion/timenote/issues) · [MIT 许可证](LICENSE)
...
- **系统**：构建目标为 macOS 14.0 及以上。**实际只在 macOS 26.6.2 / Apple Silicon 验证**，不能据此保证其他系统和其他 Mac 正常运行。
- **普通使用者不需要安装开发工具**：下载包已经包含应用、两个第三方运行库和所需资源；不需要另装 Python、Swift、数据库服务器或命令行工具。

### 获取应用

前往 [v0.1.2 macOS Release](https://github.com/Keng0nion/timenote/releases/tag/v0.1.2)，选择一种下载方式：

1. **推荐 DMG**：下载 `TimeNote-0.1.2-macOS-arm64.dmg`，打开后把“时间便签”拖到“应用程序”。
2. **也可 ZIP**：下载 `TimeNote-0.1.2-macOS-arm64.zip`，解压获得相同应用，也可以放在自己选择的文件夹。
...
0.1 / 0.1.0 / 0.1.1 换用 0.1.2 时沿用相同数据位置，不需要搬动数据库。0.1.2 没有升级数据库结构或原生 JSON 版本，也不会在升级时擅自整理旧版零散的目标归属。
...
编号用于区分不同对象，数据库关联约束用于防止记录指向不存在的目标或工作。结构变更由迁移机制管理；**0.1.2 仍沿用 `v1-local-history`，没有新增行动表或静默改写旧归属。**
...
需要 Apple Silicon Mac、现有 Apple 命令行开发工具与 macOS SDK、**Swift 6.3.3 兼容工具链**，以及 Python 3。脚本不会替你安装软件、更改系统设置或申请后台权限。
